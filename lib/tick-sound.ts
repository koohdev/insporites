...
/**
 * Lazily-initialized sound player. No `AudioContext` is created until the first
 ...
 * `prepare()`, `play()`, `playHover()`, and `playClick()` become silent no-ops.
 */
export function createTickPlayer(): TickPlayer {
  ...
  return {
    ...
    playHover() {
      if (disposed) return;
      playSynthTone(540, 780, 0.03, 0.06, "sine");
    },
    playClick() {
      if (disposed) return;
      playSynthTone(640, 220, 0.045, 0.22, "triangle");
    },
    dispose() {
      if (disposed) return;
      disposed = true;
