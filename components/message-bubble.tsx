...
// Re-export message primitives for seamless imports
export * from "@/components/agents/message";
export * from "@/components/agents/streaming-response";
...
function bubbleContentClass(
  ...
  return cn(
    "relative z-0 min-w-9 max-w-[82%] rounded-2xl px-3.5 py-2.5 text-sm leading-6 text-neutral-900 dark:text-neutral-100",
    "[&_a]:font-medium [&_a]:underline [&_a]:underline-offset-4 [&_code]:rounded [&_code]:bg-neutral-200/60 dark:[&_code]:bg-neutral-800/60 [&_code]:px-1 [&_code]:py-0.5 [&_code]:font-mono [&_code]:text-[0.9em] [&_ol]:my-2 [&_ol]:list-decimal [&_ol]:pl-5 [&_p+p]:mt-2 [&_pre]:my-2 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-neutral-200/60 dark:[&_pre]:bg-neutral-800/60 [&_pre]:p-3 [&_ul]:my-2 [&_ul]:list-disc [&_ul]:pl-5",
    variant === "solid" && "text-white dark:text-neutral-900 font-medium",
    variant === "ghost" && "w-full max-w-none rounded-none px-0 py-0 text-neutral-900 dark:text-neutral-100",
    variant === "danger" && "text-rose-600 dark:text-rose-400",
    interactive &&
      "cursor-pointer text-left outline-none transition-[background-color,color,transform] duration-150 hover:brightness-[0.98] focus-visible:ring-2 focus-visible:ring-blue-500 active:scale-[0.99]",
  );
}
...
function bubbleSurfaceClass(
  ...
  return cn(
    ...
    variant === "solid" && "bg-neutral-900 dark:bg-white",
    variant === "soft" && "bg-neutral-100 dark:bg-neutral-800 border border-neutral-200/60 dark:border-neutral-700/60",
    variant === "tint" && "bg-blue-500/10 dark:bg-blue-500/20 border border-blue-500/20",
    variant === "outline" && "border border-neutral-200 dark:border-neutral-700 bg-transparent",
    variant === "danger" && "bg-rose-500/10 dark:bg-rose-500/20 border border-rose-500/20",
  );
}
